...
int main(int argc, char *const argv[])
{
	...
	/* print the results */
	puts("");
	printf ("H2F Default Subordinate Demo\n");
	puts("");
	...
	if(h2f_bridge.abort_time == 0) {
	...
	} else if((h2f_bridge.abort_time > h2f_bridge.end_time) ||
		(h2f_bridge.abort_time < h2f_bridge.start_time)) {
		...
		/* start to abort time */
		ns_time = ((h2f_bridge.abort_time - h2f_bridge.start_time) *
						1000000000) / cntfrq_el0;
		ps_time = ((h2f_bridge.abort_time - h2f_bridge.start_time) *
						1000000000) % cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		...
		/* abort to end time */
		ns_time = ((h2f_bridge.end_time - h2f_bridge.abort_time) *
						1000000000) / cntfrq_el0;
		ps_time = ((h2f_bridge.end_time - h2f_bridge.abort_time) *
						1000000000) % cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		...
		/* start to end time */
		ns_time = ((h2f_bridge.end_time - h2f_bridge.start_time) *
						1000000000) / cntfrq_el0;
		ps_time = ((h2f_bridge.end_time - h2f_bridge.start_time) *
						1000000000) % cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
...
void do_the_interesting_thing(struct h2f_bridge_s *h2f_bridge,
	...
	*((volatile uint32_t *)(map->uio_mmap_addr +
	...
	bad_word_ptr = (uint64_t *)(h2f_1g_map->uio_mmap_addr +
			((H2F_1G_BASE + 4096) & (H2F_1G_SPAN - 1)));
...
/* --------------------------------------------------------------------------
   ...
   -------------------------------------------------------------------------- */
	...
	/* store the results */
	h2f_bridge->start_time = start_time;
	h2f_bridge->abort_time = g_abort_time;
	h2f_bridge->end_time = end_time;

#if USE_DEV_MEM_MAPPING
	result = munmap(mmap_addr, H2F_1G_SPAN);
	if(result != 0) {
		puts("ERROR: munmap");
		exit(EXIT_FAILURE);
	}
