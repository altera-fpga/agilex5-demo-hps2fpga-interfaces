...
int main(void)
{
	...
	do_the_interesting_thing(&f2h_bridge, &hps_gp_uio_map,
				&f2h_bridge_uio_map, &resmem_buf_uio_map);
...
void do_the_interesting_thing(struct f2h_bridge_s *f2h_bridge,
	...
	*((volatile uint32_t *)(map->uio_mmap_addr +
	...
	for(i = 0 ; i < F2H_OCRAM_32K_SPAN ; i++) {
		((uint8_t *)(f2h_bridge_map->uio_mmap_addr +
			((F2H_OCRAM_32K_BASE) &
	...
	temp_32 = ((volatile uint32_t *)(f2h_bridge_map->uio_mmap_addr +
		((F2H_MSGDMA_CSR_BASE) &
	...
	for(i = 0 ; i < 8 ; i++) {
		((volatile uint32_t *)(f2h_bridge_map->uio_mmap_addr +
		((F2H_MSGDMA_DES_BASE) &
...
void do_the_interesting_thing_irq(struct f2h_bridge_s *f2h_bridge,
	...
	*((volatile uint32_t *)(map->uio_mmap_addr +
	...
	temp_32 = 1;
	result = write(f2h_bridge_map->uio_fd, &temp_32, 4);
	if(result != 4) {
		f2h_bridge->error = 3;
		return;
	}
	...
	/* read the uio IRQ */
	result = read(f2h_bridge_map->uio_fd, &temp_32, 4);
	if(result != 4) {
		f2h_bridge->error = 10;
		return;
	}
