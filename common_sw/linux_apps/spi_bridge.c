...
int main(void)
{
	...
	struct uio_map_s spi_bridge_uio_map;
	...
	/* configure struct for spi-bridge-uio device */
	spi_bridge_uio_map.uio_name = "spi-bridge-uio";
	spi_bridge_uio_map.uio_length = getpagesize();
	spi_bridge_uio_map.uio_prot = PROT_READ | PROT_WRITE;
	spi_bridge_uio_map.uio_flags = MAP_SHARED;
	spi_bridge_uio_map.uio_offset = 0;

	/* find the spi-bridge-uio device */
	result = find_uio_index(&spi_bridge_uio_map);
	if(result != 0) {
		printf("ERROR: find_uio_index : %d\n", result);
		return 1;
	}

	/* open and map the spi-bridge-uio device */
	result = mmap_uio(&spi_bridge_uio_map);
	if(result != 0) {
		printf("ERROR: mmap_uio : %d\n", result);
		return 1;
	}
	...
	do_the_interesting_thing(&hps_spim, &hps_gp_uio_map, &rst_mgr_uio_map,
							&spi_bridge_uio_map);
...
void do_the_interesting_thing(struct hps_spim_s *hps_spim,
	...
	uint64_t timeout;
	uint64_t temp;
	void *per0modrst_base = (rst_mgr_map->uio_mmap_addr +
					(PER0MODRST_BASE & 0xFFF));
	void *hps_spim0_base = (spi_bridge_map->uio_mmap_addr +
					(HPS_SPIM0_BASE & 0xFFF));
...
/* signal a no-transaction command to the spi bridge hardware */
	...
	result = run_no_transaction_command(hps_spim0_base);
...
/* perform a 32-bit write and read back through spi bridge hardware */
	...
	result = spi_write_reg32(0, tx_value, hps_spim0_base);
	...
	result = spi_read_reg32(0, &rx_value, hps_spim0_base);
...
/* write 4KB and read back through the spi bridge hardware */
	...
	result = spi_write_image(0x0, &tx_pattern[0], 4 * 1024, hps_spim0_base);
	...
	for(i = 0 ; i < 4096 ; i += 4) {
		result = spi_read_reg32(i, &rx_value, hps_spim0_base);
		if(result != 0)
			break;
...
int send_spi_msg(struct spi_msg *mesg, void *hps_spim0_base);
...
int spi_write_image(uint32_t write_addr, uint8_t *write_data,
	...
	while(rem_size > 0) {
		...
		do {
			...
			result = send_spi_msg(&mesg, hps_spim0_base);
			if(result < 0)
				return 4;
		} while(i < j);
