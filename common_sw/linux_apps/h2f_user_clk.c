...
void do_the_interesting_thing(struct h2f_user_clk_s *, struct uio_map_s *);
...
int main(void)
{
	...
	do_the_interesting_thing(&h2f_user_clk, &hps_gp_uio_map);
...
void do_the_interesting_thing(struct h2f_user_clk_s *h2f_user_clk,
	...
	*((volatile uint32_t *)(map->uio_mmap_addr +
	...
	user_clk_start = *((volatile uint32_t *)(map->uio_mmap_addr +
						(HPS_GP_IN_BASE & 0xFFF)));
	...
	do {
		...
		user_clk_end = *((volatile uint32_t *)(map->uio_mmap_addr +
						(HPS_GP_IN_BASE & 0xFFF)));
