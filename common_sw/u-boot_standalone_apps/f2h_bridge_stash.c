...
void do_the_interesting_thing(struct f2h_bridge_stash_s *f2h_bridge_stash);

int f2h_bridge_stash(int argc, char *const argv[])
{
	uint64_t cntfrq_el0;
	uint64_t ns_time;
	uint64_t ps_time;
	...
	struct f2h_bridge_stash_s f2h_bridge_stash = {0};
	...
	do_the_interesting_thing(&f2h_bridge_stash);

	/* print the results */
	puts("\n");
	printf ("F2H Cache Stash Demo\n");
	puts("\n");
	...
	printf("HPS_GP_IN signature value = 0x%08X : %c%c%c%c\n",
				f2h_bridge_stash.gp_sig_value,
				(f2h_bridge_stash.gp_sig_value >>  0) & 0xFF,
				(f2h_bridge_stash.gp_sig_value >>  8) & 0xFF,
				(f2h_bridge_stash.gp_sig_value >> 16) & 0xFF,
				(f2h_bridge_stash.gp_sig_value >> 24) & 0xFF);
	puts("\n");

	printf("SYSID_ID: 0x%08X\n", f2h_bridge_stash.sysid_id);
	printf("SYSID_TS: 0x%08X\n", f2h_bridge_stash.sysid_ts);
	puts("\n");
	puts("---------- data cache flushed ----------\n");
	puts("\n");

	switch(f2h_bridge_stash.error) {
	case(0):
		puts("No errors detected...\n");
		puts("\n");
		printf("   DCACHE FLUSH TIME: ");
		ns_time = (f2h_bridge_stash.dcache_flush_time * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.dcache_flush_time * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns\n", ns_time, ps_time);
		...
		ns_time = (f2h_bridge_stash.buffer_compare_time * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.buffer_compare_time * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns\n", ns_time, ps_time);
		...
		ns_time = (f2h_bridge_stash.h2f_dma_time * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.h2f_dma_time * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		...
		ns_time = (f2h_bridge_stash.f2h_dma_time * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.f2h_dma_time * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns", ns_time, ps_time);
	...
	switch(f2h_bridge_stash.error2) {
	case(0):
		puts("No errors detected...\n");
		puts("\n");
		printf(" BUFFER COMPARE TIME: ");
		ns_time = (f2h_bridge_stash.buffer_compare_time2 * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.buffer_compare_time2 * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns\n", ns_time, ps_time);
		...
		printf("HPS to FPGA DMA TIME: ");
		ns_time = (f2h_bridge_stash.h2f_dma_time2 * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.h2f_dma_time2 * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns", ns_time, ps_time);
		...
		printf("FPGA to HPS DMA TIME: ");
		ns_time = (f2h_bridge_stash.f2h_dma_time2 * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2h_bridge_stash.f2h_dma_time2 * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns", ns_time, ps_time);
...
void do_the_interesting_thing(struct f2h_bridge_stash_s *f2h_bridge_stash) {
	...
	*((volatile uint32_t *)(HPS_GP_OUT_BASE)) = 0;
	...
	f2h_bridge_stash->sysid_id = sysid_id;
	...
	f2h_bridge_stash->sysid_ts = sysid_ts;
	...
	/* return the DMA time */
	f2h_bridge_stash->h2f_dma_time = end_time - start_time;
	...
	/* return the DMA time */
	f2h_bridge_stash->f2h_dma_time = end_time - start_time;
	...
	if(memcmp(dma_buffer_0, dma_buffer_1, F2H_OCRAM_32K_SPAN) == 0) {
		f2h_bridge_stash->error = 0;
	} else {
		f2h_bridge_stash->error = 6;
	}
	...
	f2h_bridge_stash->buffer_compare_time = end_time - start_time;
	...
	/* return the DMA time */
	f2h_bridge_stash->h2f_dma_time2 = end_time - start_time;
	...
	/* return the DMA time */
	f2h_bridge_stash->f2h_dma_time2 = end_time - start_time;
	...
	/* verify the initial buffer matches the final buffer */
	if(memcmp(dma_buffer_0, dma_buffer_1, F2H_OCRAM_32K_SPAN) == 0) {
		f2h_bridge_stash->error2 = 0;
	} else {
		f2h_bridge_stash->error2 = 6;
	}
	/* mark the time */
	asm volatile (
		"dsb sy\n"
	...
	/* return the buffer compare time */
	f2h_bridge_stash->buffer_compare_time2 = end_time - start_time;
}
