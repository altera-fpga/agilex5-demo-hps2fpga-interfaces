...
int f2sdram_bridge(int argc, char *const argv[])
{
	uint64_t cntfrq_el0;
	uint64_t ns_time;
	uint64_t ps_time;
	...
	do_the_interesting_thing(&f2sdram_bridge);
	...
	case(0):
		...
		ns_time = (f2sdram_bridge.dcache_flush_time * 1000000000)
								/ cntfrq_el0;
		ps_time = (f2sdram_bridge.dcache_flush_time * 1000000000)
								% cntfrq_el0;
		ps_time *= 1000;
		ps_time /= cntfrq_el0;
		printf("%9lld.%03lldns\n", ns_time, ps_time);
...
void do_the_interesting_thing(struct f2sdram_bridge_s *f2sdram_bridge) {
	...
	uint64_t temp;
	uint32_t temp_32;
	uint8_t *dma_buffer_0 = (void*)(F2SDRAM_BUF_0_BASE);
	uint8_t *dma_buffer_1 = (void*)(F2SDRAM_BUF_1_BASE);
	...
	*((volatile uint32_t *)(HPS_GP_OUT_BASE)) = 0;
	...
	f2sdram_bridge->dcache_flush_time = end_time - start_time;
